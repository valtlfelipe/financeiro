...
Keep positive transaction and series amounts, known enum values, valid transfer source/destination shapes, paired installment metadata, valid recurrence shapes, nonzero installment allocations, and ordered series dates protected by database constraints. These are PostgreSQL CHECK constraints. PostgreSQL is the only supported driver, in production and in the test suite, so there is no second dialect to keep in sync.
